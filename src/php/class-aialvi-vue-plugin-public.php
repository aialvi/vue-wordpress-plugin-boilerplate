...
class AIALVI_Vue_Plugin_Public {
	...
	public function render_shortcode( $atts ) {
		...
		$output .= '<div class="loading">' . esc_html__( 'Loading...', 'aialvi-page-ranks' ) . '</div>';
		$output .= '</div>';
	...
	public function handle_public_ajax() {
		...
		$nonce_field = sanitize_text_field( wp_unslash( $_POST['nonce'] ?? $_GET['nonce'] ?? '' ) );
		...
		$action = sanitize_text_field( wp_unslash( $_POST['sub_action'] ?? $_GET['sub_action'] ?? '' ) );
		...
		switch ( $action ) {
			...
			default:
				wp_send_json_error(
					array(
						'message' => esc_html__( 'Invalid action', 'aialvi-page-ranks' ),
					)
				);
				break;
		}
	...
	private function handle_form_submission() {
		...
		if ( ! wp_verify_nonce( $form_nonce, 'aialvi_vue_form_nonce' ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Form security check failed', 'aialvi-page-ranks' ),
				)
			);
		}
		...
		if ( isset( $_POST['form_data'] ) && is_array( $_POST['form_data'] ) ) {
			...
			$unslashed_form_data = wp_unslash( $_POST['form_data'] );
			foreach ( $unslashed_form_data as $key => $value ) {
				...
				wp_send_json_error(
					array(
						'message' => sprintf(
							...
							esc_html__( 'Field %s is required', 'aialvi-page-ranks' ),
							esc_html( $field )
						),
					)
				);
		...
		if ( ! is_email( $form_data['email'] ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Please enter a valid email address', 'aialvi-page-ranks' ),
				)
			);
		}
		...
		if ( $result ) {
			wp_send_json_success(
				array(
					'message' => esc_html__( 'Form submitted successfully', 'aialvi-page-ranks' ),
				)
			);
		} else {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Failed to process form', 'aialvi-page-ranks' ),
				)
			);
		}
	}
	 ...
	 */
	public function rest_get_public_data() {
		$data = array(
			'items'     => $this->get_public_items(),
			'timestamp' => time(),
		);
	...
	public function rest_submit_form( $request ) {
		...
		if ( $result ) {
			...
			return new WP_Error( 'submit_failed', esc_html__( 'Failed to process form', 'aialvi-page-ranks' ), array( 'status' => 500 ) );
		}
	}
