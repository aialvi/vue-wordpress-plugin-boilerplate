...
class AIALVI_Vue_Plugin_Security
{
    ...
    {
        if (!is_array($data)) {
            return new WP_Error('invalid_data', __('Data must be an array', 'aialvi-vue-plugin'));
        }
    ...
    public static function validate_file_upload($file, $allowed_types = array(), $max_size = 2097152)
    {
        ...
        if (!isset($file['tmp_name']) || !is_uploaded_file($file['tmp_name'])) {
            return new WP_Error('invalid_upload', __('Invalid file upload', 'aialvi-vue-plugin'));
        }

        // Check file size
        if ($file['size'] > $max_size) {
            return new WP_Error('file_too_large', __('File size exceeds limit', 'aialvi-vue-plugin'));
        }
        ...
        if (!empty($allowed_types) && !in_array($file_type['type'], $allowed_types, true)) {
            return new WP_Error('invalid_file_type', __('File type not allowed', 'aialvi-vue-plugin'));
        }
        ...
        if (in_array(strtolower($file_extension), $dangerous_extensions, true)) {
            return new WP_Error('dangerous_file', __('File type not allowed for security reasons', 'aialvi-vue-plugin'));
        }
    ...
    public static function log_security_event($event, $context = array())
    {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG && defined('WP_DEBUG_LOG') && WP_DEBUG_LOG) {
            error_log('AIALVI Security Event: ' . wp_json_encode($log_entry));
        }
    ...
    /**
     * Clean old security log entries
     */
    public static function cleanup_security_logs($days_to_keep = 30)
    {
        global $wpdb;
